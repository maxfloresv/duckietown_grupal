...
class Template(object):
	...
	def __init__(self, args):
		...
		# Extras:
		self.instrucciones = []
		# -1 cuando no se ejecuta, y 1 cuando si se ejecuta
		self.run = -1
		self.vel_lineal = 0
		self.vel_angular = 0
		self.zero = 0
		...
		while True:
			self.xd()
		"""
			len_inst = len(self.instrucciones)
			if len_inst != 0:
				primera = self.instrucciones[0]
				ultima = self.instrucciones[len_inst - 1]
	...
	def ejecutar_instruccion(self, texto, instruccion, v_lin, v_ang, t):
		...
		if distancia <= MAX_DIST:
			...
			while time() - t_actual <= t:
				print(self.instrucciones[len(self.instrucciones)-1])
				#print(instruccion, freno)
				if freno == True:
					print("frenafrena")
					msg_rueda.v = 0
					msg_rueda.omega = 0
	...
	def callback_voz(self, msg):
		print("avdsc")
		global ult_instruccion
		ult_instruccion = msg.data
		self.run = 1
	...
	def xd(self):
		...
		# Se "muere" la instruccion anterior
		self.run = -1
		...
		if bailar_dist <= MAX_DIST:
			eng.say("dale a tu cuerpo alegria macarena")
			eng.runAndWait()
